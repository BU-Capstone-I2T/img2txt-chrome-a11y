...
let feedbackEnabled = true; // TODO: Add a settings toggle for this
...
/**
 * Submits the feedback form for an image. The form allows users to provide feedback on the alt text
 * generated by the AI.
 *
 * @param {Event} event
 ...
 * @param {HTMLSpanElement} feedbackMsgSpan
 */
const submitFeedbackForm = (event, img, input, errorSpan, feedbackMsgSpan) => {
    event.preventDefault();

    // Reset messages
    errorSpan.textContent = '';
    feedbackMsgSpan.textContent = '';

    // Validate the input
    if (isNaN(input.value) || input.value < 1 || input.value > 5) {
        errorSpan.textContent = 'Please enter a number between 1 and 5';
    ...
    // Send the feedback to the service worker
    const message = {
    ...
    feedbackMsgSpan.textContent = `You rated this image's description ${input.value} out of 5`;
    feedbackMsgSpan.hidden = false;

    input.value = ''; // Clear the input field after submission
}

/**
 * Adds a feedback form to an image node. The form allows users to provide feedback on the alt text
 * generated by the AI.
 *
 * @param {HTMLImageElement} imgNode
 */
const addFeedbackFormToImageNode = (imgNode) => {
    const uuid = crypto.randomUUID();
    const originalParent = imgNode.parentElement;
    const form = document.createElement('form');
    form.id = `feedbackForm-${uuid}`;
    form.className = 'sr-only';
    const label = document.createElement('label');
    label.htmlFor = `feedbackInput-${uuid}`;
    label.textContent = 'Alt-Text AI Description Feedback Score, 1 to 5';
    const input = document.createElement('input');
    input.id = `feedbackInput-${uuid}`;
    input.type = 'number';
    input.min = '1';
    input.max = '5';
    input.placeholder = 'Enter a number 1 to 5';
    input.required = true;
    const errorSpan = document.createElement('span');
    errorSpan.setAttribute('aria-live', 'assertive');
    errorSpan.setAttribute('aria-atomic', 'true');
    errorSpan.hidden = true;
    const feedbackMsgSpan = document.createElement('span');
    feedbackMsgSpan.setAttribute('aria-live', 'assertive');
    feedbackMsgSpan.setAttribute('aria-atomic', 'true');
    feedbackMsgSpan.hidden = true;

    form.appendChild(label);
    form.appendChild(input);
    form.appendChild(errorSpan);
    form.appendChild(feedbackMsgSpan);
    form.addEventListener('submit', (event) => {
        submitFeedbackForm(event, imgNode, input, errorSpan, feedbackMsgSpan);
    });
 ...
 */
const addTextElementToImageNode = (imgNode, textContent) => {
    const originalParent = imgNode.parentElement;
...
const sendImageToServiceWorker = (img, drawableImg) => {
    ...
    chrome.runtime.sendMessage(message, (response) => {
        ...
        if (feedbackEnabled) {
            addFeedbackFormToImageNode(img);
        }
    });
};
